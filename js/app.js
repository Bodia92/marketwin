(() => {
    ...
    function spollers() {
        ...
        const spollersBody = document.querySelectorAll(".spollers__body");
        if (spollersArray.length > 0 && spollersBody.length > 0) {
    ...
    swiper_core_Swiper.use([ Navigation, Pagination, EffectFade, Controller ]);
    function initSliders() {
        ...
        if (document.querySelector(".product__swiper")) new swiper_core_Swiper(".product__swiper", {
            modules: [ Pagination ],
            observer: true,
            observeParents: true,
            slidesPerView: 3,
        ...
        if (document.querySelector(".full-screen__slider")) {
            new swiper_core_Swiper(".full-screen__slider", {
                modules: [ Navigation ],
                observer: true,
                observeParents: true,
                slidesPerView: 1,
    ...
    document.addEventListener("DOMContentLoaded", (() => {
        ...
        if (images.length > 0) {
            images.forEach((img => img.setAttribute("hidden", true)));
        ...
        if (video && videoWrap && videoBtn) {
            ...
            videoBtn.addEventListener("click", (() => video.dispatchEvent(new Event("click"))));
        }
        ...
        if (brandGroups.length > 0 && alphabetFilter) {
            ...
            brandGroups.forEach((group => {
                ...
                if (filter) filterSet.add(filter.toUpperCase());
            }));
            const sortedFilters = Array.from(filterSet).sort();
            sortedFilters.forEach((letter => {
                const li = document.createElement("li");
                li.className = "alphabet-item";
                li.setAttribute("data-filter", letter);
                li.textContent = letter;
                alphabetFilter.appendChild(li);
            }));
            alphabetFilter.querySelectorAll("[data-filter]").forEach((btn => {
                btn.addEventListener("click", (() => {
                    const filter = btn.getAttribute("data-filter");
                    const current = alphabetFilter.querySelector("[data-filter].active");
                    if (!filter) return;
                    if (current?.getAttribute("data-filter") === filter) {
                        btn.classList.remove("active");
                        brandGroups.forEach((item => item.classList.remove("d-none")));
                    } else {
                        current?.classList.remove("active");
                        btn.classList.add("active");
                        brandGroups.forEach((item => {
                            const isAllow = item.getAttribute("data-filter") === filter;
                            item.classList.toggle("d-none", !isAllow);
                        }));
                    }
                }));
            }));
        ...
        const filterBrands = () => {
            ...
            brandCards.forEach((card => {
                const title = (card.getAttribute("js-filter-title") || "").toLowerCase();
                card.classList.toggle("d-none", !title.includes(term));
            }));
            brandGroupsGrid.forEach((group => {
                const visibleItems = group.querySelectorAll(".item-partners:not(.d-none)");
                const groupWrapper = group.closest(".brand-group");
                if (groupWrapper) groupWrapper.classList.toggle("d-none", visibleItems.length === 0);
            }));
        };
